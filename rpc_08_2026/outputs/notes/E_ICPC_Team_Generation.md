...
Se tienen $N$ competidores ordenados por rango ($1$ a $N$). Cada competidor en rango $r$ tiene un intervalo $[a_r, b_r]$ tal que solo acepta formar equipo con competidores cuyo rango esté dentro de dicho intervalo ($a_r \le r \le b_r$). Cada equipo debe tener exactamente 3 integrantes.
La restricción dorada del enunciado es:
$$\text{Para todo } i < j \implies a_i \le a_j \quad \text{y} \quad b_i \le b_j$$
...
2. Para que el trío consecutivo $\{i, i+1, i+2\}$ sea mutuamente compatible, basta con que el competidor más a la izquierda acepte al de más a la derecha ($b_i \ge i+2$) y el competidor más a la derecha acepte al de más a la izquierda ($a_{i+2} \le i$). Las condiciones de $i+1$ quedan automáticamente satisfechas por la transitividad monótona ($a_{i+1} \le a_{i+2} \le i$ y $b_{i+1} \ge b_i \ge i+2$).
...
2. **Ignorar la propiedad de orden monótono ($a_i \le a_j$ y $b_i \le b_j$):**  
...
- **Complejidad:** Ambos logran $O(N)$ en tiempo y $O(N)$ en espacio con $N \le 50$.
