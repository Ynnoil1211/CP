...
1. **Día $i$:** Se recolecta hasta una cantidad $q_i$ de comida. Esta comida tiene una fecha de caducidad estricta: solo puede consumirse hasta la noche del turno $f_i$ ($i \le f_i \le N$). Al pasar la noche $f_i$, cualquier porción restante se pudre.
2. **Noche $i$:** Cada uno de los $K$ jugadores debe consumir exactamente una cantidad $x$ de alimento. Si en cualquier noche algún jugador no recibe su ración completa $x$, todos pierden. Por lo tanto, el grupo necesita consumir en total $F = K \cdot x$ unidades de comida cada noche.
...
- **Monotonía de la Solución (Predicado Monótono):**  
  ...
  Esta propiedad monótona es la señal inequívoca para **Búsqueda Binaria sobre la Respuesta (Binary Search on Answer / BSTA)** en el espacio continuo $[0, \text{total\_food} / K]$.
...
Dada una demanda total requerida por noche $F = K \cdot x$:
1. Mantenemos una cola de prioridad con pares $(\text{vencimiento}, \text{cantidad\_disponible})$.
2. Para cada día $i = 0, 1, \dots, N-1$:
   - Insertamos el alimento recolectado hoy: $(f_i, q_i)$ si $q_i > 0$.
   - Necesitamos cubrir una demanda restante $\text{today} = F$.
   - Mientras $\text{today} > 0$ y la cola no esté vacía:
     - Extraemos el elemento con menor fecha de vencimiento.
     - Si ya venció (vencimiento $< i$), lo descartamos sin usarlo.
     - Si su cantidad $\le \text{today}$, lo consumimos por completo, restamos su cantidad a $\text{today}$ y lo retiramos del heap.
     - Si su cantidad $> \text{today}$, consumimos la fracción $\text{today}$, actualizamos su cantidad remanente y lo volvemos a insertar en el heap con $\text{today} = 0$.
   - Si al finalizar el intento de consumo $\text{today} > 10^{-9}$, significa que no hubo comida suficiente para la noche $i$: retornamos `false`.
...
La solución oficial (`inputs/official_solutions/C_Hunger.cpp`) implementa 150 iteraciones de bisección. Con 150 pasos, el intervalo se reduce por $2^{150} \approx 10^{45}$, superando con holgura la precisión requerida ($10^{-9}$).
...
- **Tiempo:** En cada llamada a `canDo`, cada porción de comida entra y sale de la cola de prioridad a lo sumo una vez por día, resultando en $O(N \log N)$.  
  Con $I \approx 80$ iteraciones de bisección: $O(I \cdot N \log N)$. Para $N = 10^6$, en C++ con I/O optimizada corre holgadamente en el límite de tiempo de 3.0s.
