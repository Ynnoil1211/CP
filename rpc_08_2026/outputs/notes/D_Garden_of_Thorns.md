...
Eddy posee un jardín rectangular en el plano cartesiano delimitado por [0, w] * [0, h].
En el jardín hay N plantas específicas ubicadas en coordenadas (x_i, y_i), cada una con un valor intrínseco v_i.
Su asistente Zyra planta un círculo de espinas de radio r centrado en un punto aleatorio (X, Y) distribuido de manera uniforme continua en el interior del jardín [0, w] * [0, h].
Una planta i queda protegida si cae estrictamente dentro del círculo de espinas; es decir:
dist((X, Y), (x_i, y_i)) = sqrt((X - x_i)^2 + (Y - y_i)^2) < r

**Objetivo:** Calcular el valor esperado del total de plantas protegidas: E[sum_{i=1}^N v_i * 1[{planta i protegida\]}], con un error absoluto o relativo no mayor a 10^-9.

---

## 2. Patrones Evidentes y Pistas en las Restricciones

- **Restricción diminuta de plantas (1 <= N <= 10):**  
...
- **Linealidad de la Esperanza:**  
  ...
  E[V] = sum_{i=1}^N v_i * P(planta i queda protegida)
...
- **Geometrización de la Probabilidad:**  
  La condición dist((X, Y), (x_i, y_i)) < r es equivalente a decir que el centro aleatorio (X, Y) debe pertenecer al disco abierto D((x_i, y_i), r) de radio r centrado en la planta (x_i, y_i).
  Dado que el centro (X, Y) se escoge con distribución uniforme en el rectángulo R = [0, w] * [0, h] de área w * h:
  P(planta i queda protegida) = (Área(D((x_i, y_i), r) ∩ [0, w] * [0, h])) / (w * h)
  El problema se reduce puramente a: **Calcular el área de intersección entre un círculo de radio r centrado en (x_i, y_i) y un rectángulo alineado con los ejes [0, w] * [0, h]**.
...
1. **Círculo Completamente Contenido:**
   Si x >= r AND y >= r AND w - x >= r AND h - y >= r, el disco está totalmente dentro del rectángulo:
   Área = pi r^2
2. **Rectángulo Completamente Contenido en el Círculo:**
   Si las 4 esquinas del rectángulo están a distancia <= r del centro:
   Área = w * h
...
2. **Suma de Regiones:**  
   ...
   - **Sectores circulares** ((1)/(2) r^2 delta_theta) correspondientes a los arcos que no son cortados por los bordes y conectan los extremos de las proyecciones.
...
En `getArea(x, y, r)`:
- Verifica los casos base triviales (pi r^2 o w * h).
- Proyecta triángulos en los 4 bordes donde la distancia a la pared es menor que r (h-y < r, x < r, y < r, w-x < r) sumando el área del triángulo (x_2 - x_1) * dist * (1)/(2).
- Colecciona los puntos de corte en una lista circular `pts`, calcula los ángulos angulares vía `Math.atan2` y suma los sectores circulares intermedios: (1)/(2) r^2 (alpha_2 - alpha_1).

### Complejidad
- **Tiempo:** O(N) operaciones con N <= 10. El tiempo de cálculo es imperceptible (< 0.01 s).
