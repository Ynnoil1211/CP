...
N = D * T <=> D es un divisor exacto de N (N (mod D) == 0)

Las restricciones del tratamiento imponen dos límites:
1. **Dosis máxima por pastilla (K):** La pastilla diaria no puede exceder K mg => D <= K.
2. **Duración máxima del tratamiento (P):** El tratamiento no puede durar más de P días => T = (N)/(D) <= P.

Se pide imprimir cuántos valores distintos de D cumplen las condiciones y listarlos en orden estrictamente creciente.
Dado que N <= 10^12, los divisores de N se encuentran en tiempo O(sqrt(N)) iterando i desde 1 hasta floor(sqrt(N)). Para cada i tal que N (mod i) == 0, se prueban los dos divisores conjugados d_1 = i y d_2 = N / i.
...
1. **Versiones en Python (`Magnesium.py` y `(1).py`): TLE por bucle lineal:**
   ...
   ```
   Con K, N <= 10^12, un bucle hasta 10^12 es inviable en 1 segundo (se requieren O(10^8) operaciones por segundo máximo). Provocó Time Limit Exceeded.
...
3. **`Magnesium (2).cpp`: WA por Desbordamiento de Enteros (Integer Overflow):**
   ...
   ```
   La lógica algorítmica era perfecta (O(sqrt(N))), pero `vt` se declaró como `vector<int>`. Al insertar divisores mayores a 2 * 10^9, los números se truncaron a valores negativos o corruptos.
...
2. **Confundir el tamaño de la respuesta con el tamaño del espacio de búsqueda:**  
   Al ver K <= 10^12, suponer que se puede iterar hasta K. La divisibilidad es una propiedad de N, no de K. El espacio de divisores de N tiene a lo sumo ≈ 6720 divisores para N <= 10^12, y solo requiere buscar hasta sqrt(N) <= 10^6.
...
- **Oficial ([H_Magnesium.py](../../inputs/official_solutions/H_Magnesium.py) - Arup Guha):**
  ...
  ```
- **Complejidad:** O(sqrt(N) + D(N) log D(N)) tiempo (donde D(N) <= 6720) y O(D(N)) espacio. Tiempo de ejecución < 0.05 s.