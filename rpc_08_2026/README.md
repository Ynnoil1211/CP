...
| **C** | [Don't Hunger Together](outputs/notes/C_Dont_Hunger_Together.md) | *Upsolved* | Oficial | Div 1B / Div 2E | Búsqueda Binaria sobre la Respuesta + Min-Heap (EDF) | $O(I \cdot N \log N)$ |
...
| **I** | [Missing Number](outputs/notes/I_Missing_Number.md) | *Upsolved* | Oficial | Div 2C | Fuerza Bruta sobre Prefijo ($L \le 5$) + Simulación | $O(\sum \|S\|)$ |
...
- **Manifestación:** En `Lines.cpp`, el código incluía `if (sum > 500) return i;`. El número `500` provenía del primer caso de ejemplo donde $5 \text{ horas} \times 100 \text{ líneas/hora} = 500$.
...
- **Manifestación:** En `Magnesium (2).cpp`, las variables se leían como `ll n, y, z;` (hasta $10^{12}$), pero el contenedor de almacenamiento era `vi vt;` (`vector<int>`, donde `int` soporta hasta $2 \cdot 10^9$). Al almacenar los divisores conjugados $N/i \le 10^{12}$, ocurrieron desbordamientos negativos inmediatos.
...
- **Manifestación:** Los intentos iniciales en Python iteraban `range(1, k+1)` con $K \le 10^{12}$, produciendo TLE fulminante.
- **Causa Raíz:** Fijarse en la cota de la pastilla ($K$) en lugar de explotar la propiedad estructural de divisibilidad de $N$, que solo requiere explorar hasta $\sqrt{N} \le 10^6$.
...
- **Manifestación:** En `Water.cpp`, el código estructuraba `if (tiene_min) ... else cout << a;`. Se asumía erróneamente que si faltaba el mínimo, el máximo debía estar presente obligatoriamente. Se omitió el caso donde **ambos** estaban ausentes en el registro con $a \ne b$, situación en la cual es imposible cubrir ambos con un solo día restante (debía responder `-1`).
...
- **Patrón:** Procesar cadenas donde las transiciones abarcan ventanas cortas ($L \in [3, 5]$).
...
- **Lección:** Descomponer la esperanza total en la suma de probabilidades individuales $\mathbb{E}[\sum V_i] = \sum V_i \mathbb{P}(E_i)$, transformando el problema en el cálculo del área de corte entre un círculo y un rectángulo alineado.
...
- **Lección:** Cuando la cota del valor inicial es pequeña ($A \le 99\,999$), el número de dígitos iniciales está acotado por 5. Probar cada longitud inicial convierte todo el resto del problema en una verificación determinista sin bifurcaciones.
...
- **Lección:** Todo operador lógico de conjuntos sobre dos eventos se reduce a calcular $|A|$, $|B|$ y $|A \cap B|$. Con doble hash polinomial, estas tres cantidades se indexan en $O(1)$ por longitud de patrón.
