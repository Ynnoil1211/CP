...
4. **Integer Overflow**:
   - Range sum queries can easily exceed $2 \times 10^9$ when $N \le 2 \times 10^5$ and $A_i \le 10^9$. Always type `tree`, `lazy`, and query return values as `long long`.

### Graphs & Trees
1. **BFS Queue Mark Invariant**:
   - Never mark nodes when popped from the queue. If node $v$ has multiple incoming edges from the current BFS frontier, it will be pushed into the queue multiple times, leading to $O(V^2)$ or $O(E \cdot V)$ blowup and TLE/MLE.
