...
"Find maximum MST weight after deleting any single edge in a 2-edge-connected graph (N <= 10^5, M <= 10^6)." Deleting an edge e in an MST splits the tree into two connected components requiring a minimal replacement cross-edge.
...
- Remove (1-2): replacement is (1-3, w=5) => 3 - 1 + 5 = 7.
- Remove (2-3): replacement is (1-3, w=5) => 3 - 2 + 5 = 6.
