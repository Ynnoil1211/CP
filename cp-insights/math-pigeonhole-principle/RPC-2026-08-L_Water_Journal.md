...
💡 One missing day cannot simultaneously cover two missing extremes (a != b with neither recorded); if both appear, any value in [a, b] is valid, otherwise the missing extreme is uniquely forced.
...
Evaluate the four-state boolean matrix (has_a, has_b); with only 1 missing slot, needing both distinct extremes is an immediate contradiction by the pigeonhole principle (k > 1 requirements, 1 slot).
